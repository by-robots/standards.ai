...
- Secure defaults.
- Authentication and authorisation is correct.
- Data is handled securely.
- Dependencies are from trusted sources.

Security and simplicity are not exclusive — suggest solutions that achieve both.
...
- Write clear, readable code. Favour clarity over cleverness.
- Keep methods/functions short and single-purpose.
- Name things well. If a name needs a comment to explain it, rename it.
- Follow existing patterns in the codebase. Consistency matters more than personal preference.
- Handle errors explicitly. Do not silently swallow exceptions or leave unhappy paths unhandled.
...
Discourage and avoid the following:

- Over-abstraction, especially when it obscures security.
- Unnecessary complexity, especially when it obscures security.
- Code that sacrifices readability.
- Hard-coded secrets or sensitive configuration.
- Broad permissions or overly permissive access controls.
...
- Follow existing project conventions (framework, module style, etc.).
...
- Follow existing project conventions for file structure and nesting.
...
- Be mindful of memory usage with large collections — prefer
  `find_each` / `find_in_batches` over `all.each`.

### Logging

- Never log sensitive data such as tokens, passwords, API keys, or PII.
- Log meaningfully — include enough context to be useful for debugging
  without leaking security-sensitive information.
...
- Code should be self-documenting. Use clear naming over comments.
- Add inline comments sparingly — only for non-obvious logic, decisions, or workarounds.
...
- Prefer well-maintained, trusted gems and packages. Check for recent
  activity and known vulnerabilities before suggesting.
