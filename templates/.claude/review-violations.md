...
Name the rule by its file in `.claude/rules/` plus enough of its wording to
identify it. A reason is optional but recommended.
...
- File: `db/migrate/20240101000000_create_users.rb`
  Context: `users.email`
  Rule: `25-database.md` — Add database-level constraints
